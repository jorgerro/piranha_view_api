class Booking < ActiveRecord::Base
  ...
  private
    ...
    def validate_booking
      ...
      # there is a special case when there is more than one shared boat and each one has capacity larger than the request? 
      ...
      p "new_bookings", new_bookings
      p "shared_subs", shared_subs
      ...
      shared_subs.each do |sub|
        ...
        next unless current_availability >= self.size
          ...
          if !fewest_boats && !smallest_leftover_availability
            ...
            selected_sub = sub
            p "selected_sub", selected_sub
            p "fewest_boats", fewest_boats
            p "smallest_leftover_availability", smallest_leftover_availability
            next
          end
        ...
        # remove other assignments that use the selected shared boats
        selected_sub_ids = selected_sub.map(&:id)
        p "SELECTED SUB:", selected_sub
